            ...
            {categories.map((c, index) => {
              return(  <button key={index} 
                            onClick={updateFilters} 
                             type="button" name='category' className={`${category === c.toLowerCase() ? 'active' : null}`}>{c}</button>
                          )
                            })}
          </div>
        </div>
          ...
          <div>
           ...
           <select name='company' value={company} onChange={updateFilters} className='company'>
            {companies.map((c, index) => {
              return <option key={index} value={c}></option>
            })}
            </select>  
          </div> 
        {/* end of companies */}
         ...
         <div className='form-control'>
           ...
           <div className='colors'>
            {colors.map((c, index) => {
              if(c === 'all'){
                return <button name='color' onClick={updateFilters} data-color='all' className={`${color === 'all' ? 'color-btn active' : 'colo-btn'}`}>all</button>
              }
              return(
